...
useEffect(() => {
  ...
  useEffect(() => {
    if (quizSubmitted) {
      ...
      const t1 = setTimeout(() => { setShowConfetti(false); setConfettiRunning(false); }, 10000);
      const t2 = setTimeout(() => { setConfettiVisible(false); }, 15000);
  ...
  useEffect(() => {
    if (!questions || questions.length === 0) {
      ...
      fetch(`${API_BASE_URL}/question/category/${category}`)
        ...
        .catch(err => { console.log("error in getting questions", err); setLoading(false); });
  ...
  const handleSubmit = async finalScore => {
    try {
      const response = await fetch(`${API_BASE_URL}/question/saveQuiz`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: user, category:category , score: finalScore })
      });
        ...
        <div className="relative w-full max-w-4xl">
          ...
          <div className="bg-gray-700 w-full text-white rounded-xl p-10">
            <div className="mb-10 border-black rounded-sm p-2 text-center bg-gray-200">
              <p className="text-lg md:text-xl text-black">{currentIndex + 1}. {currentQuestion.question}</p>
            </div>
            <div className="grid grid-cols-2 gap-6 text-lg mb-6">
              {[currentQuestion.option1,currentQuestion.option2,currentQuestion.option3,currentQuestion.option4].map((opt, idx) => (
                <p key={idx} onClick={() => handleOptionClick(opt)} className={`cursor-pointer text-black rounded-sm text-center py-2
